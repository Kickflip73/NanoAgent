...
function nonStrictToolSchema(schema: {
...
const observeToolJsonSchema = nonStrictToolSchema(z.toJSONSchema(observeToolParameters));
const actToolJsonSchema = nonStrictToolSchema(z.toJSONSchema(actToolParameters));
...
export function createComputerTools(
  ...
  const observe = tool({
    ...
    parameters: observeToolJsonSchema,
    strict: false,
