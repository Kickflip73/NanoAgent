...
test('preserves mode and permission tool policy semantics', () => {
  ...
  assert.deepEqual(
    ...
    ['read_file', 'http_get', 'update_plan', 'unknown_extension'],
  );
  assert.deepEqual(
...
test('classifies the existing durable side-effect tools from one policy source', () => {
  const sideEffects = [
    'write_file', 'edit_file', 'move_file', 'run_shell', 'http_request', 'index_knowledge',
  ...
  for (const name of ['read_file', 'http_get', 'recall', 'show_plan', 'delegate_review', 'unknown_extension']) {
...
test('derives displayed orchestration tools from mode policy', () => {
  ...
  assert.deepEqual(toolNamesForMode('general', base), [
    'delegate_research', 'delegate_review', 'read_file', 'write_file',
  ]);
  ...
  assert.deepEqual(toolNamesForMode('ultra', base), [
    'claim_team_task', 'delegate_architecture', 'delegate_research', 'delegate_review',
