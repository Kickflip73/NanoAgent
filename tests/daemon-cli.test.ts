...
test('daemon CLI exposes thin dead-letter list, retry and archive controls', async () => {
  const root = await mkdtemp(path.join(os.tmpdir(), 'mimi-cli-dead-letter-'));
  const localConfig = { ...config, daemonDataRoot: root };
  const calls: Array<{ method: string; params: unknown }> = [];
  const server = new MimiIpcServer(path.join(root, 'mimi.sock'), (method, params) => {
  ...
  process.stdout.write = (() => true) as typeof process.stdout.write;
  try {
    await runDaemonCommand(localConfig, ['activity', '5']);
    await runDaemonCommand(localConfig, ['outbox', '7']);
    await runDaemonCommand(localConfig, ['show', 'event', 'event-detail']);
    await runDaemonCommand(localConfig, ['show', 'run', 'run-detail']);
    await runDaemonCommand(localConfig, ['show', 'outbox', 'outbox-detail']);
    await runDaemonCommand(localConfig, ['show', 'schedule', 'schedule-detail']);
    await runDaemonCommand(localConfig, ['retry', 'event', 'event-1']);
    await runDaemonCommand(localConfig, ['retry', 'outbox', 'outbox-1']);
    await runDaemonCommand(localConfig, ['archive', 'event', 'event-2']);
    await runDaemonCommand(localConfig, ['archive', 'outbox', 'outbox-2']);
  } finally {
    process.stdout.write = write;
    await server.close();
  }
  assert.deepEqual(calls, [
    { method: 'activity.get', params: { limit: 5 } },
    { method: 'outbox.list', params: { limit: 7 } },
    { method: 'event.get', params: { id: 'event-detail' } },
    { method: 'run.get', params: { id: 'run-detail' } },
    { method: 'outbox.get', params: { id: 'outbox-detail' } },
    { method: 'schedule.get', params: { id: 'schedule-detail' } },
    { method: 'event.retry', params: { id: 'event-1' } },
    { method: 'outbox.retry', params: { id: 'outbox-1' } },
    { method: 'event.archive', params: { id: 'event-2' } },
    { method: 'outbox.archive', params: { id: 'outbox-2' } },
  ]);
  await assert.rejects(runDaemonCommand(localConfig, ['retry', 'run', 'id']), /仅支持 event 或 outbox/);
  await assert.rejects(runDaemonCommand(localConfig, ['archive', 'event']), /请提供要归档/);
  await assert.rejects(runDaemonCommand(localConfig, ['show', 'memory', 'id']), /仅支持/);
  await assert.rejects(runDaemonCommand(localConfig, ['show', 'event']), /请提供要查看/);
  assert.match(daemonHelp(), /retry outbox/);
  assert.match(daemonHelp(), /daemon activity/);
  assert.match(daemonHelp(), /daemon show/);
});
