...
它面向真实工作提供 Agent Loop、多模型、工具调用、Token-aware 上下文、持久会话、长期记忆、NANO.md 持久指令、Agent Skills、MCP、本地 RAG、Goal/Resume 和受控 SubAgent。三种运行模式分别覆盖日常任务、只读方案讨论和大型并行任务；Ultra Team 使用本地依赖任务板与最多四个隔离 worker，不引入多进程编排平台。NanoAgent 刻意不实现消息网关、任意深度多 Agent 图、复杂权限平台和分布式任务。
...
- 单个主 Agent、单进程、本地优先；Team worker 只有一层且由 lead 统一整合。
- 每个模块只承担一个清晰职责。
