...
Panda Draw note: this bundled copy is used inside the self-contained `panda-draw` skill. The visual style is fixed to `morandi-journal`; use this framework mainly to classify information structure, layout needs, learning objectives, complexity, audience, and source facts. Do not route to other styles or external baoyu skills.
