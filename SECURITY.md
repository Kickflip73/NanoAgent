...
NanoAgent 是学习项目，不应直接作为面向不可信用户的生产服务。

## 权限模型

`run_shell`、文件工具和 MCP Server 使用当前操作系统用户权限运行。`http_request` 可以访问当前网络环境中的 HTTP 服务。模型可能读取、修改或移动文件、访问内部地址，也可能执行外部内容中包含的不安全指令。
