...
- LLOneBot/LLBot/NapCat/OneBot 都是个人 QQ 的非官方路径，不等同于腾讯官方 API，可能触发账号风控并修改 QQ Electron 运行时。即使桌面共存模式不创建第二个登录设备，也只应使用可审核、与当前 QQ 构建兼容的插件，并固定 loopback HTTP/WS、强 token 和单一反向 WebSocket 上游；升级 QQ 或插件前先回归，不能接受时改用能力范围更窄的腾讯官方 QQ Bot。独立 NapCat 的 macOS 安装器仍要求 GitHub Release SHA-256 digest、兼容 QQ build、完整代码签名、Apple 执行策略、腾讯 Team ID 和已知入口同时成立，保留精确备份并提供 `restore`；其 LaunchAgent 在普通 QQ 正运行、入口被重置或 Shell 缺失时失败关闭。
