...
首次运行 `mimi` 会自动从发布包内的 `mimi.connectors.example.json` 创建 `~/.mimi-agent/daemon/connectors.json`，将 Node 和 Connector 脚本转换为当前安装位置的绝对路径，并在 macOS 上默认启用十一个本机 Connector。需要 Token 或额外数据源配置的大象、QQ、OpenClaw 微信、Radar 和 File Radar 默认关闭。QQ/微信不会默认启用 AppleScript、截图、OCR、键盘或点击式 Connector；正式 IM 接入必须使用后台 API/协议桥。后续初始化会补齐缺失的 enabled 默认本机 Connector，但不会加入默认关闭的外部通道；同 ID、仍指向 canonical 内置脚本的 Connector 会补充发布包新增的 action。已有 enabled、命令、参数、环境白名单、来源和 owner action 描述保持不变。写入是原子的，无新增项时不改文件。也可用 `MIMI_CONNECTORS_CONFIG` 指向其他绝对配置文件。
...
仓库提供 `examples/connectors/qq-napcat-connector.mjs`，通过 NapCatQQ（基于 NTQQ 的非官方协议框架）接入个人 QQ，支持私聊和群聊消息的后台接收、回复、目录和有界历史查询。NapCat 不是腾讯官方个人号 API，存在账号风控、客户端版本兼容和第三方供应链风险；不能接受该风险时，应改用腾讯官方 QQ Bot，但其能力范围不是个人 QQ 收件箱。

**工作原理：** Connector 启动 WebSocket 服务，NapCat 作为客户端连接推送 OneBot 11 事件。收到消息事件后转为 MimiAgent 协议输出到 stdout。MimiAgent 的 deliver 指令通过 NapCat HTTP API 发送回 QQ。
...
"qq": {
  ...
  "envAllowlist": ["NC_HTTP_URL", "NC_WS_PORT", "NC_ACCESS_TOKEN", "NC_WS_ACCESS_TOKEN", "NC_STATUS_POLL_MS"],
  "source": "qq",
  "trust": "external",
  "profileId": "owner",
  "restart": true,
  ...
  "actions": {
    ...
    "health_check": { "description": "检查 NapCat HTTP 状态与反向 WebSocket 入站连接" },
...
- `MACOS_SYSTEM_POLL_INTERVAL_MS`：默认 60000，`0` 关闭主动 Event 但保留 action。
...
`examples/connectors/macos-life-connector.mjs` 通过 macOS 自带 JXA/`osascript` 接入 Calendar、Reminders 和 Notification Center，不需要额外 npm 依赖。开启实际配置 `~/.mimi-agent/daemon/connectors.json` 中的 `macos-life` 后，首次调用会由 macOS 请求日历、提醒事项或自动化访问权限；MimiAgent 不再叠加自己的审批。
...
修改动作的标题上限 1000 字符、地点 5000、备注 40000，priority 必须是系统字典定义的 0～9，日期必须可解析。空更新、无效字段类型和找不到稳定 ID 都明确失败，不按名称猜测对象。删除 recurring Calendar event 的 occurrence/series 语义由 Calendar.app 决定，Connector 不引入 recurrence DSL。所有写操作继续使用 Action Bridge 的不确定结果不自动重放保护。
...
Connector 通过 argv 向 `osascript` 传 JSON，不用 Shell 拼接日程标题或备注。如果守护进程由 launchd 启动，macOS 隐私授权对应的是实际 Node/Terminal 可执行程序；无法访问时先在前台运行一次 Connector 完成系统授权。
