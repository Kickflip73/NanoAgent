...
3. Memory、RAG、Plan、Goal、Session 并行读取
4. ContextManager 按 Token Budget 选择近期完整轮次
5. 较早历史、Skill Catalog、Memory、RAG、Goal 被组装为动态 Instructions
...
只有成功连接的 Server 会交给主 Agent 和 SubAgent。远程认证保持在环境变量中，不应写入 `mcp.json`。
