...
普通 Task 在终态事务中登记有 provenance 的 Memory observation；达到 10 条或最老等待 10 分钟后，统一 Task Scheduler 才发布 `priority: 0` 的 `memory_maintenance` Task。页面变化通过 compilation receipt 幂等计数，累计 50 页或有变化且 7 天未 lint 时，把 semantic lint 合并到下一 Task；成功终态才清零，dead-letter 保留计数。维护 Run 使用固定 profile Session，每批最多 20 条、8KB 证据和 5 页 upsert，只可使用 Wiki 读工具及三项内部 receipt 工具，不可使用 Shell、文件、网络、MCP、Connector、Schedule 或委派。单条 external/public observation 的 active 晋级由 Host 硬拒绝；Task dead-letter 时 observation 保持 pending，owner 可用 `/memory maintain` 重试或执行有界 semantic lint。
...
Wiki 页面使用严格 YAML frontmatter、SourceRef、窄主题正文和单页 atomic rename。Compiler ingest 先写 pending CompilationPlan/receipt，页面落盘后更新 FTS/vector/links，最后生成 `_index.md`、追加 `_log.md` 并完成 receipt；重复 digest 返回同一结果。查询先检索 private/workspace Wiki，SQLite FTS5/BM25 为始终可用基线，可选 Embedding vector 以 RRF 合并；Provider 失败立即回退词法通道。短中文查询使用 bounded LIKE/别名回退，外部修改的无效页面不进入 active index并出现在 lint，来源 digest 变化的页面标记 stale。
