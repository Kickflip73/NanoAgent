...
本地 Function Tool 的副作用以 `sessionId + runId + toolName + callId` 记入执行账本。相同成功调用返回已保存结果；`started` 或 `failed` 状态不会自动重试，以免重复写文件、命令或外部请求。Daemon Run 还在原生 MCP transport 边界使用同一 executionKey 做语义去重；Hosted Tools 仍不在本地账本控制内。这是 at-most-once 防护，不宣称跨任意崩溃边界的 exactly-once。
...
`finish_task` 只提交候选终态。Host 会把引用的 tool call 与 ExecutionLedger 中的真实结果、调用参数、产物写入、测试退出码和当前 Goal 的未完成 Plan 逐项核对；客观条件必须预先绑定工具和关键参数片段，复合任务必须覆盖产物与外部回执的证据并集。首份 Contract 建立后不可重写降级，旧 Report/Gate 也不会串到新任务。未通过项返回当前 Run 继续处理，模型仍结束时由 `completeRun` 再次拒绝终态并由 Daemon 按指数退避可靠续跑。外部动作只有结构化 `outcome=confirmed` 回执能满足完成条件，`accepted`、超时、断连或未知结果都视为不确定，禁止宣称成功和自动重放。后台 `blocked` 还必须成功调用 `request_background_task_input`，由 Host 持久化后才成立。

Completion Contract、报告和最近门控结果随 Run checkpoint 持久化；Goal 只能由通过的 Completion Gate 标记完成，模型不能直接写入 completed。
