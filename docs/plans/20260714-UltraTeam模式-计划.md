...
将 NanoAgent 模式收敛为 General、Plan、Ultra Team；让 Plan 具备真实只读讨论边界，让 Ultra Team 通过多个专门 SubAgent、依赖任务板、有限并发、Goal checkpoint 和验证门禁完成大规模代码与其他长程任务。
...
**涉及文件：** `src/runtime/instructions.ts`、`src/runtime/nano-agent.ts`、`src/commands.ts`
...
**涉及文件：** `src/runtime/tool-policy.ts`、`src/runtime/nano-agent.ts`
...
数据按 Session 存入 `.nano-agent/teams.json`，沿用串行写队列和临时文件 rename，避免并发覆盖。
...
**涉及文件：** `src/extensions/team.ts`、`src/extensions/subagents.ts`、`src/runtime/nano-agent.ts`
...
**涉及文件：** `tests/*`、`evals/agent-cases.json`、`README.md`、`docs/ARCHITECTURE.md`、`CHANGELOG.md`、`knowledge/nano-agent.md`
...
9. README、ARCHITECTURE、NANO.md、knowledge、CHANGELOG 与 CLI 帮助保持一致。
